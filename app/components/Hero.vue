<template>
  <main class="relative w-full min-h-screen overflow-hidden bg-background-light">
    <!-- Wave Animation Background — confined to the bottom -->
    <div class="absolute inset-x-0 bottom-0 h-[45%] w-full z-0 overflow-hidden">
    ...
    <!-- Gradient overlay to soften wave-to-cream transition -->
    <div
      class="absolute inset-x-0 bottom-0 h-[55%] bg-gradient-to-b from-background-light via-background-light/80 to-transparent z-[1] pointer-events-none">
    </div>
    ...
    <div class="relative z-10 flex flex-col items-center justify-center min-h-screen px-6 md:px-12 py-12">
      ...
      <!-- Hero Content — centered in a rounded card -->
      <div
        class="w-full max-w-[900px] mx-auto bg-white/40 backdrop-blur-xl rounded-[32px] md:rounded-[48px] border border-white/60 shadow-[0_8px_60px_-12px_rgba(0,0,0,0.08)] px-8 md:px-14 py-10 md:py-14 text-center space-y-6 mt-16 mb-8">
        <h1
        ...
        <div class="w-full max-w-md mx-auto pt-2">
          <form class="relative" id="join" @submit.prevent="handleSubmit">
            <input v-model="email"
              class="w-full rounded-2xl py-3.5 pl-6 pr-14 text-brand-charcoal font-medium placeholder-brand-charcoal/35 outline-none font-sans text-sm transition-all duration-200 bg-white/50 backdrop-blur-lg border border-brand-charcoal/8 shadow-[0_2px_16px_-4px_rgba(0,0,0,0.06)] hover:shadow-[0_4px_24px_-4px_rgba(0,0,0,0.1)] focus:shadow-[0_4px_28px_-4px_rgba(0,0,0,0.12)] focus:border-brand-charcoal/15 focus:bg-white/70"
...
.wave-background {
  ...
  width: 100%;
  height: 100%;
  background: linear-gradient(135deg, #c4817f 0%, #a85250 40%, #810100 100%);
  transition: 0.5s;
}
...
.wave-layer-2 {
  border-radius: 40%;
  background: rgba(237, 234, 220, 0.5);
  animation: wave-animate 14s linear infinite;
}

.wave-layer-3 {
  border-radius: 42.5%;
  background: rgba(237, 234, 220, 0.3);
